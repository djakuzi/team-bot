export class MemoryCalc {
    private static toSerializable(value: unknown, seen = new WeakSet<object>()): unknown {
        ...
        if (type === "object") {
            ...
            if (value instanceof Map) {
                return Array.from(value.entries()).map(([k, v]) =>
                    [this.toSerializable(k, seen), this.toSerializable(v, seen)]
                );
            }
            ...
            for (const key in value as Record<string, unknown>) {
                result[key] = this.toSerializable((value as Record<string, unknown>)[key], seen);
            }
            return result;
        }
