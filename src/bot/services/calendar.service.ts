...
import {typeUpdateBot} from '@tb-bot/types/context.type';
...
export class ServiceBotCalendar implements OnModuleInit {
  ...
  onModuleInit() {
    const Calendar =
      CalendarLib as unknown as typeof import('telegraf-calendar-telegram').Calendar;
    ...
    this.bot.action(/calendar-telegram-prev-[\d-]+/, ctx =>
      this.actionPrevOrDirection(ctx, 'prev'),
    );
    this.bot.action(/calendar-telegram-next-[\d-]+/, ctx =>
      this.actionPrevOrDirection(ctx, 'next'),
    );
  }
  ...
  private async actionPrevOrDirection(
    ctx: typeUpdateBot,
    direction: 'prev' | 'next',
  ) {
    const dateString = ctx.match[0].replace(
      `calendar-telegram-${direction}-`,
      '',
    );
    const date = new Date(dateString);
    date.setMonth(
      direction === 'prev' ? date.getMonth() - 1 : date.getMonth() + 1,
    );
