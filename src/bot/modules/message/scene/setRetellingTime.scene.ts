...
export class SceneRetellingTime extends ASceneBot {
    cancelScene = async (ctx: Scenes.WizardContext, message: string) => {
        await buildCancelScene(ctx, message, HANDLERS_BOT_MESSAGE.cancelChangeTimeRetelling);
    }
    ...
    async onCancel(@Ctx() ctx: Scenes.WizardContext) {
        await super.onCancel(ctx, 'Изменение времени генерации пересказа сообщений отменено.')
    }

    @WizardStep(1)
    async ask(@Ctx() ctx: Scenes.WizardContext) {
        await this.cancelScene(ctx, 'Введите время генерации пересказа сообщений в формате ЧЧ:ММ (например, 14:30)');
    ...
    async askTime(@Ctx() ctx: Scenes.WizardContext) {
        ...
        await ctx.reply(`Время генерации пересказа сообщений установлено на ${time}`);
