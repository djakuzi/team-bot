...
export class SceneSetToneTime extends ASceneBot {
  cancelScene = async (ctx: Scenes.WizardContext, message: string) => {
    await buildCancelScene(
      ctx,
      message,
      HANDLERS_BOT_TONE.cancelChangeTimeTone,
    );
  };

  constructor(private readonly serviceToneSettings: ServiceToneSettings) {
    super();
  }
  ...
  async ask(@Ctx() ctx: Scenes.WizardContext) {
    await this.cancelScene(
      ctx,
      'Введите время обновления тона в формате ЧЧ:ММ (например, 14:30)',
    );
