...
import { cancelChangeTimeUpdatedTone } from './handlers/cancelChangeTimeUpdatedTone.handler';
...
export class OnBotTone {
	...
	async onCancel(ctx: Scenes.WizardContext, next: () => Promise<void>) {
		...
		if (data === HANDLERS_BOT_TONE.cancelChangeTimeUpdatedTone.name) {
			await cancelChangeTimeUpdatedTone(ctx);
			return;
		}
