...
import { ACTIONS_BOT_MENU } from '@tb-bot/menu/constant/actions.const';
...
export class ActionsBotSettings {
	...
	async getSizeCache(@Ctx() ctx: Context) {
		await getSizeCache(ctx, this.serviceCache, 'action');
	}

	@Action(ACTIONS_BOT_SETTINGS.сlearCache.action)
	async сlearCache(@Ctx() ctx: Context) {
		await сlearCache(ctx, this.serviceCache, 'action');
	}
}
