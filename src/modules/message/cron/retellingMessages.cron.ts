...
export class CronRetellingMessages implements OnModuleInit {
    ...
    private async scheduleRetellingMessage(time: string) {
        ...
        this.logger.log(`Произошло обновление времени с ${this.timeRetelling} на ${time} по интервальному пересказу сообщений.`);
    ...
    private async callbackUpdateTone() {
        this.logger.log(`Генерация пересказа началась по расписанию в ${this.timeRetelling}.`);
        ...
        this.logger.log(`Генерация пересказа начавшееся по расписанию в ${this.timeRetelling} закончилась.`);
    }
